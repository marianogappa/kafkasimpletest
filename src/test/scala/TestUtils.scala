...
object TestUtils extends Logging {
  ...
  def checkEquals(b1: ByteBuffer, b2: ByteBuffer) {
    assert(b1.limit - b1.position == b2.limit - b2.position, "Buffers should have equal length")
    for(i <- 0 until b1.limit - b1.position)
      assert(b1.get(b1.position + i) == b2.get(b1.position + i), "byte " + i + " byte not equal.")
  }
  ...
  def checkEquals[T](expected: Iterator[T], actual: Iterator[T]) {
    ...
    if (expected.hasNext) {
      ...
      assert(false, "Iterators have uneven length-- first has more: "+length1 + " > " + length)
    }
    ...
    if (actual.hasNext) {
      ...
      assert(false, "Iterators have uneven length-- second has more: "+length2 + " > " + length)
    }
  }
  ...
  def checkEquals[T](s1: java.util.Iterator[T], s2: java.util.Iterator[T]) {
    ...
    assert(!s1.hasNext, "Iterators have uneven length--first has more")
    assert(!s2.hasNext, "Iterators have uneven length--second has more")
  }
  ...
  def waitUntilLeaderIsElectedOrChanged(zkClient: ZkClient, topic: String, partition: Int, timeoutMs: Long = 5000L,
    ...
    if (!isLeaderElectedOrChanged)
      throw new RuntimeException("Timing out after %d ms since leader is not elected or changed for partition [%s,%d]") // MG: replaced a JUnit fail
  ...
  def waitUntilTrue(condition: () => Boolean, msg: String, waitTime: Long = 5000L): Boolean = {
    ...
    while (true) {
      ...
      if (System.currentTimeMillis() > startTime + waitTime)
        throw new RuntimeException(msg) // MG: replaced a JUnit fail
  ...
  def checkForPhantomInSyncReplicas(zkClient: ZkClient, topic: String, partitionToBeReassigned: Int, assignedReplicas: Seq[Int]) {
    ...
    assert(phantomInSyncReplicas.size == 0,
